...
public class PurchaseOrderSequence {
	...
	public static void main(String args[]) {
		...
		orders.add(new Order(3, 6002, 101));
		orders.add(new Order(4, 6003, 100));
		orders.add(new Order(5, 6005, 100));
		orders.add(new Order(6, 6006, 100));
		orders.add(new Order(7, 6006, 102));
		orders.add(new Order(8, 6005, 108));
	...
	public GroupsInfo groupByDestination(List<Order> orders) {
		...
		orders.stream().forEach(o -> {
			originDestinationSetMap.computeIfAbsent(o.getOrigin(), HashSet::new);
		...
		return new GroupsInfo(originDestinationSetMap, destinationGroupList);
	}
	...
	public void populateGroupEndPoints(GroupsInfo groupsInfo) {
		...
		for (int i = 0; i < destinationGroupList.size(); i++) {
			...
			if (destinationGroup.getEndpoints().size() < 2) {
				// if less than 2 origins belong to more than 1 group. In this case any unique origin can be selected as an endpoint
				destinationGroup.getEndpoints().addAll(destinationGroup.getUniqueOrigins().stream()
